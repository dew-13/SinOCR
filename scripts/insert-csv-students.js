/**
 * CSV to Database Insertion Script
...
// For Neon database connection (adjust if using different DB)
...
async function insertStudents() {
  try {
    ...
    if (!tableExists[0].exists) {
      throw new Error('Students table does not exist. Please run schema creation scripts first.');
    }
      ...
      try {
        ...
        await sql`
          INSERT INTO students (
            ...
            ${student.status === 'active' ? 'Active' : student.status === 'employed' ? 'Employed' : 'Pending'},
